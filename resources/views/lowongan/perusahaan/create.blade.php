...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            ...
            <form class="form" action="{{ route('perusahaan.info-lowongan.store') }}" method="POST">
                @csrf
                <div class="row">
                    ...
                    <div class="mb-3 col-md-6 col-12">
                        <div class="form-group mandatory">
                            <label for="jenis-waktu-pekerjaan"
                                class="form-label @error('jenis-waktu-pekerjaan') is-invalid @enderror">Jenis Waktu
                                Pekerjaan</label>
                            <select name="jenis-waktu-pekerjaan" id="jenis-waktu-pekerjaan"
                                ...
                                @foreach (['Waktu Kerja Standar (Full-Time)', 'Waktu Kerja Paruh Waktu (Part-Time)', 'Waktu Kerja Fleksibel (Flexible Hours)', 'Shift Kerja (Shift Work)', 'Waktu Kerja Bergilir (Rotating Shifts)', 'Waktu Kerja Jarak Jauh (Remote Work)', 'Waktu Kerja Kontrak (Contract Work)', 'Waktu Kerja Proyek (Project-Based Work)', 'Waktu Kerja Tidak Teratur (Irregular Hours)', 'Waktu Kerja Sementara (Temporary Work)'] as $jenis)
                                    <option value="{{ $jenis }}"
                                        {{ old('jenis-waktu-pekerjaan') == $jenis ? 'selected' : null }}>
                                        {{ $jenis }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-3 col-12">
                        <div class="form-group mandatory">
                            ...
                            <textarea class="form-control @error('deskripsi') is-invalid @enderror" id="deskripsi" rows="5" name="deskripsi">{{ old('deskripsi') }}</textarea>
                            @error('deskripsi')
                                <span class="invalid-feedback d-block mt-2" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
