...
@section('content')
    <div class="card">
            ...
            <div class="row align-items-center">
                ...
                <div class="col-2 col-md-3 text-end">
                    @if (!Route::is('alumni.profil.riwayat-pendidikan-non-formal.create'))
                        ...
                        <form id="destroy-form" action="{{ route('alumni.profil.riwayat-pendidikan-non-formal.destroy', $pendidikanNonFormal->id_riwayat_pendidikan_non_formal) }}" method="POST" class="d-none">
                            @csrf @method('DELETE')
                        </form>
                    @endif
                </div>
        ...
        <div class="card-body">
            ...
            <form class="form"
                @if (!Route::is('alumni.profil.riwayat-pendidikan-non-formal.create'))
                    action="{{route ('alumni.profil.riwayat-pendidikan-non-formal.update', $pendidikanNonFormal->id_riwayat_pendidikan_non_formal)}}"
                @else
                    action="{{route ('alumni.profil.riwayat-pendidikan-non-formal.store')}}"
                @endif
                ...
                <div class="row">
                    <div class="mb-3 col-md-6 col-12">
                        <x-input type="text" name="nama-lembaga" label="Nama Lembaga"
                            placeholder="Nama Lembaga" value="{{ $pendidikanNonFormal->nama_lembaga ?? '' }}"
                            class="mandatory" required="true" />
                    </div>
                    <div class="col-md-6 col-12">
                        <x-input type="text" name="alamat-lengkap" label="Alamat Lengkap"
                            placeholder="Alamat Lengkap" value="{{ $alamat['alamat-lengkap'] ?? '' }}"
                            class="mandatory" required="true" />
                    </div>
                    <div class="col-md-6 col-12">
                    ...
                    <div class="mb-3 col-12 form-group">
                        <x-input type="text" name="nama-kursus" label="Nama Kursus"
                            placeholder="Nama Kursus" value="{{ $pendidikanNonFormal->nama_kursus ?? '' }}"
                            class="mandatory" required="true" />
                    </div>
                    <div class="mb-3 col-12">
                        <x-input type="text" name="tanggal" label="Tanggal"
                            placeholder="Pilih Tanggal" value="{{ $pendidikanNonFormal->tanggal ?? '' }}"
                            class="mandatory" required="true" />
                    </div>
                </div>
