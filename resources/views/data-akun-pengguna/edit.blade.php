            ...
            <div class="row mt-4">
                <div class="col-md-6 col-12 mt-2">
                    <form action="{{ route('admin.akun-pengguna.update', $user->username) }}" method="POST"
                        ...
                        @method('PUT')
                        <button type="submit"
                            class="btn btn-success w-100 {{ $user->admin->status == 'Aktif' ? 'disabled' : '' }}">
                            <i class="bi bi-check-circle me-1"></i>Aktif
                        </button>
                    </form>
                </div>
                <div class="col-md-6 col-12 mt-2">
                    <form action="{{ route('admin.akun-pengguna.update', $user->username) }}" method="POST" class="w-100 m-1">
                        <input type="hidden" name="status" value="Tidak Aktif">
                        @csrf
                        @method('PUT')
                        <button type="submit"
                            class="btn btn-danger w-100 {{ $user->admin->status == 'Tidak Aktif' ? 'disabled' : '' }}">
